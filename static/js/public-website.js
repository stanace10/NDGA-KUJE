(() => {
  ...
  if (deferredVideos.length) {
    ...
    const shouldLoadAmbientVideo = !reducedMotion && !connection?.saveData && window.innerWidth >= 900;
    if (shouldLoadAmbientVideo) {
      const scheduleVideoLoad = () => {
        window.setTimeout(() => deferredVideos.forEach(loadDeferredVideo), 1800);
      };
  ...
  if (form) {
    ...
    const saveButtons = [...form.querySelectorAll("[data-save-progress]")];
    const saveStatus = document.querySelector("[data-registration-save-status]");
    const progressBar = document.querySelector("[data-registration-progress]");
    const stepSummary = document.querySelector("[data-registration-step-summary]");
    const indicators = [...document.querySelectorAll("[data-step-indicator]")];
    const saveKey = `ndga-registration-progress:${window.location.pathname}`;
    let stepIndex = 0;
    const persistableFields = () =>
      [...form.querySelectorAll("input, select, textarea")].filter((field) => {
        if (!field.name || field.disabled || field.type === "hidden" || field.type === "file") return false;
        return true;
      });
    ...
    const saveProgress = () => {
      try {
        window.localStorage.setItem(saveKey, JSON.stringify(snapshotForm()));
        setSaveStatus("Progress saved on this device. You can come back and continue later.");
      } catch (error) {
        setSaveStatus("This browser did not allow local saving, so progress may not stay on this device.");
      }
    };
    const restoreProgress = () => {
      try {
        ...
        const restoredStep = Number(payload?.stepIndex);
        if (Number.isFinite(restoredStep) && restoredStep >= 0 && restoredStep < steps.length) {
          stepIndex = restoredStep;
        }
        setSaveStatus("Saved progress was restored on this device.");
      } catch (error) {
        setSaveStatus("Saved progress could not be restored, but you can continue from here.");
      }
    };
    const stepIsValid = () => {
      let valid = true;
    ...
    const updateSteps = () => {
      ...
      if (stepSummary) {
        stepSummary.textContent = `Step ${stepIndex + 1} of ${steps.length}`;
      }
    ...
    saveButtons.forEach((button) => button.addEventListener("click", saveProgress));
    persistableFields().forEach((field) => {
