...
def _add_img_attr(tag, name, value):
    if re.search(rf"\s{name}\s*=", tag, flags=re.IGNORECASE):
        return tag
    return tag[:-1] + f' {name}="{value}">'


def _prepare_inline_media(html):
    def replace(match):
        tag = match.group(0)
        tag = _add_img_attr(tag, "loading", "lazy")
        tag = _add_img_attr(tag, "decoding", "async")
        return _add_img_attr(tag, "fetchpriority", "low")
...
def cbt_notation(value, autoescape=True):
    ...
    return mark_safe(_prepare_inline_media("".join(formatted)))
