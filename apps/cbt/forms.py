...
class QuestionBankCreateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["assignment"].queryset = authoring_assignment_queryset(self.actor)
...
class ExamCreateForm(forms.Form):
    ...
    def clean(self):
        ...
        if manual:
            ...
            exam_limit = Decimal("100.00") if _uses_ss3_mock_exam_totals(assignment) else Decimal("50.00")
...
class ExamUploadImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["source_file"].help_text = (
            ...
            "The local parser works without paid AI credit; difficult blocks can use configured AI repair. "
            "Legacy DOC is not accepted here."
        )
    ...
    def clean(self):
        ...
        if start and end and end <= start:
            self.add_error("schedule_end", "Closing time must be after opening time.")
...
class ITExamActivationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.exam is not None:
            ...
            self.fields["schedule_start"].initial = (
                timezone.localtime(self.exam.schedule_start).strftime("%Y-%m-%dT%H:%M")
                if self.exam.schedule_start
                else None
            )
            self.fields["schedule_end"].initial = (
                timezone.localtime(self.exam.schedule_end).strftime("%Y-%m-%dT%H:%M")
                if self.exam.schedule_end
                else None
            )
