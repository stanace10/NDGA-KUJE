...
class PortalAccessMiddleware:
    ...
    CLOUD_HIDDEN_PORTAL_KEYS = {"staff", "dean", "form", "hostel", "it", "bursar", "vp", "principal", "cbt", "election"}
    ...
    ROLE_GUARDED_PATH_PREFIXES = {
        ...
        "/results/hostel/": {ROLE_HOSTEL_SUPERVISOR, ROLE_IT_MANAGER, ROLE_PRINCIPAL},
        "/results/report/academic-performance/": {ROLE_IT_MANAGER},
        "/results/report/ca-performance/": {ROLE_IT_MANAGER},
        "/results/management/ca1/": {ROLE_IT_MANAGER},
        "/results/report/": {
            ROLE_DEAN,
            ROLE_IT_MANAGER,
            ROLE_VP,
        ...
        "/cbt/dean/": {ROLE_DEAN, ROLE_IT_MANAGER},
        ...
        "/portal/hostel/": PORTAL_ROLE_ACCESS["hostel"],
    ...
    def _enforce_cloud_student_portal_restrictions(self, request):
        ...
        context = {
            ...
            "restriction_message": (
                ...
                "classroom/LMS, transcript, published results, and notifications while internal authoring "
            ...
            "allowed_summary": (
                ...
                "weekly challenge, classroom/LMS view, published result view, and notifications."
            ),
            "current_node_label": "Cloud",
        }
        return render(request, "dashboard/lan_restricted.html", context=context, status=403)
    ...
    def _enforce_future_term_staff_edit_lock(self, request):
        ...
        term_name = lock_state.get("term_name") or "The active term"
        start_date_label = start_date.strftime("%B %d, %Y").replace(" 0", " ")
        lock_message = (
            f"{term_name} has not opened yet. Staff can continue to view earlier records "
            "through filters, but entry, marking, and attendance actions remain locked "
            f"until {start_date_label}."
        )
        if request.method in {"GET", "HEAD", "OPTIONS"}:
            request.term_edit_locked = True
            request.term_edit_lock_start_date = start_date
            request.term_edit_lock_message = lock_message
            return None
        context = {
            "restriction_title": "Term Entry Locked",
            "restriction_heading": "Staff editing is closed until the new term opens.",
            "restriction_message": lock_message,
            "allowed_summary": (
                f"{term_name} begins on {start_date_label}. "
    ...
    def _login_url(self, request, *, fresh=False):
        portal_key = request.portal_key
        if request.path.startswith(("/portal/student/", "/finance/student/")):
            portal_key = "student"
        query = {"next": request.path, "audience": portal_key}
        if fresh:
            query["fresh"] = "1"
        return build_portal_url(request, portal_key, reverse("accounts:login"), query)
