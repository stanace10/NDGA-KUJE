...
def cloud_student_finance_disabled():
    return bool(getattr(settings, "CLOUD_STUDENT_FINANCE_DISABLED", False) and sync_node_role() == "CLOUD")
...
def path_is_cloud_student_portal_limited(path):
    ...
    if (
        ...
        and any(request_path.startswith(prefix) for prefix in CLOUD_STUDENT_FINANCE_PREFIXES)
    ):
        return True
    if any(request_path.startswith(prefix) for prefix in CLOUD_STUDENT_FINANCE_PREFIXES):
        return False
...
def current_term_staff_edit_lock():
    try:
        ...
        return {"locked": False, "start_date": None, "term_name": ""}
    ...
    if calendar is None or not calendar.start_date:
        return {
            "locked": False,
            "start_date": None,
            "term_name": setup_state.current_term.get_name_display(),
        }
    today = timezone.localdate()
    return {
        "locked": calendar.start_date > today,
        "start_date": calendar.start_date,
        "term_name": calendar.term.get_name_display(),
    }
