...
class StageTwoHostRoutingTests(TestCase):
    ...
    def test_staff_shortcut_redirects_to_staff_portal(self):
        client = Client(HTTP_HOST="192.168.10.10")
        response = client.get("/staff/")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "/portal/staff/")

    def test_subject_teacher_cannot_login_from_cbt_portal(self):
        client = Client(HTTP_HOST="cbt.ndgakuje.org")
        response = client.post(
            "/auth/login/?audience=cbt",
            {"username": "teacher_host", "password": "Password123!"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn("staff.ndgakuje.org/auth/login/", response.url)
    ...
    def test_localhost_election_shortcut_redirects_to_portal_entry(self):
        client = Client(HTTP_HOST="localhost:8000")
        response = client.get("/election/")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "/portal/election/")
    ...
    @override_settings(
        ...
        CLOUD_STUDENT_FINANCE_DISABLED=True,
    )
    def test_cloud_student_portal_hides_finance_until_payments_are_ready(self):
        client = Client(HTTP_HOST="localhost:8000")
    ...
    def test_cloud_student_portal_blocks_lan_only_learning_features(self):
        ...
        blocked_finance = client.get("/portal/student/finance/")
        self.assertEqual(blocked_finance.status_code, 403)
