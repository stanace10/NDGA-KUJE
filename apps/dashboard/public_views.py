...
from apps.finance.prospectus import prospectus_for_class
...
class PublicSiteEnabledMixin:
    ...
    def base_context(self):
        ...
        context["pixpay_school_fees_url"] = getattr(settings, "PIXPAY_SCHOOL_FEES_URL", "")
        context["admission_payment_bank_name"] = getattr(settings, "ADMISSION_PAYMENT_BANK_NAME", "")
        context["admission_payment_account_number"] = getattr(settings, "ADMISSION_PAYMENT_ACCOUNT_NUMBER", "")
        context["admission_payment_account_name"] = getattr(settings, "ADMISSION_PAYMENT_ACCOUNT_NAME", "")
        context["admission_application_fee_amount"] = getattr(settings, "ADMISSION_APPLICATION_FEE_AMOUNT", 0)
        return context
...
class PublicContentPageView(PublicSiteEnabledMixin, TemplateView):
    ...
    def _build_listing_context(self, *, items, filter_key, query_param):
        filter_options = []
        seen = set()
        for item in items:
            value = (item.get(filter_key) or "").strip()
            if value and value not in seen:
                seen.add(value)
                filter_options.append(value)

        active_filter = (self.request.GET.get(query_param, "") or "").strip()
        if active_filter and active_filter in seen:
            filtered_items = [item for item in items if (item.get(filter_key) or "").strip() == active_filter]
        else:
            active_filter = ""
            filtered_items = list(items)

        paginator = Paginator(filtered_items, self.listing_page_size)
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.page_slug == "gallery":
            ...
            listing = self._build_listing_context(
                items=get_public_news(),
                filter_key="category",
                query_param="category",
            )
            context.update(
                {
                    "news_items": listing["items"],
                    "news_page_obj": listing["page_obj"],
                    "news_filter_options": listing["filter_options"],
                    "news_active_filter": listing["active_filter"],
                    "news_filtered_count": listing["filtered_count"],
                    "news_total_count": listing["total_count"],
                    "news_query_param": listing["query_param"],
                }
            )
        elif self.page_slug == "events":
            listing = self._build_listing_context(
                items=get_public_events(),
                filter_key="meta",
                query_param="meta",
            )
            context.update(
                {
                    "event_items": listing["items"],
                    "event_page_obj": listing["page_obj"],
                    "event_filter_options": listing["filter_options"],
                    "event_active_filter": listing["active_filter"],
                    "event_filtered_count": listing["filtered_count"],
                    "event_total_count": listing["total_count"],
                    "event_query_param": listing["query_param"],
                }
            )
        return context
...
class PublicEventDetailView(PublicSiteEnabledMixin, TemplateView):
    template_name = "website/event_detail.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        event = get_public_event_item(kwargs["slug"])
        if event is None:
            raise Http404()
        context.update(self.base_context())
        context.update(
            {
                ...
                "page_description": event["summary"],
                "event_item": event,
                "related_events": [
                    row for row in get_public_events() if row["slug"] != event["slug"]
                ][:2],
            }
        )
        return context
...
class PublicRegistrationView(PublicSiteEnabledMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return redirect(_public_admission_status_url(request=request, submission=submission))
...
class PublicRegistrationStatusView(PublicSiteEnabledMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        prospectus_fee_schedule = prospectus_for_class(
            admission_snapshot["school_placement"].get("intended_class") or self.submission.intended_class,
            new_intake=True,
        )
        ...
        context.update(
            {
                ...
                "prospectus_fee_schedule": prospectus_fee_schedule,
    ...
    def post(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(init_error="Online admission payment is no longer active. Please pay into the admission account and upload/keep your receipt for bursary confirmation.")
        )
...
class PublicAdmissionFormDownloadView(PublicSiteEnabledMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if not submission.public_admission_pdf_available():
            return redirect(
                f"{_public_admission_status_url(request=request, submission=submission)}&error=Admission+PDF+is+released+after+bursary+confirms+the+admission+application+fee."
            )
