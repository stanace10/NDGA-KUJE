...
from apps.dashboard.image_utils import optimize_public_upload
...
class AcademicWindowUpdateForm(_StyledFormMixin, forms.ModelForm):
    class Meta:
        model = AcademicOperationWindow
        fields = ("is_enabled", "start_at", "end_at", "note")
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["is_enabled"].required = False
        ...
        if self.instance and self.instance.start_at:
            self.initial["start_at"] = timezone.localtime(self.instance.start_at).strftime("%Y-%m-%dT%H:%M")
        if self.instance and self.instance.end_at:
            self.initial["end_at"] = timezone.localtime(self.instance.end_at).strftime("%Y-%m-%dT%H:%M")
    ...
    def clean(self):
        ...
        if is_enabled and (not start_at or not end_at):
            raise forms.ValidationError("Set both start and end time before opening this window.")
        if start_at and end_at and end_at <= start_at:
            raise forms.ValidationError("Window end time must be after the start time.")
        return cleaned
...
class PublicGalleryCategoryForm(_StyledFormMixin, forms.ModelForm):
    ...
    def clean_cover_image(self):
        return optimize_public_upload(self.cleaned_data.get("cover_image"), max_size=(1400, 900))
...
class PublicGalleryImageForm(_StyledFormMixin, forms.ModelForm):
    ...
    def clean_image(self):
        return optimize_public_upload(self.cleaned_data.get("image"), max_size=(1600, 1100))
...
class PublicNewsPostForm(_StyledFormMixin, forms.ModelForm):
    ...
    def clean_image(self):
        return optimize_public_upload(self.cleaned_data.get("image"), max_size=(1600, 1000))
...
class PublicEventImageForm(_StyledFormMixin, forms.ModelForm):
    class Meta:
        model = PublicEventImage
        fields = ("event", "title", "caption", "image", "sort_order", "is_active")

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["event"].queryset = PublicEventPost.objects.order_by("sort_order", "-event_date", "title")
        self.fields["image"].widget.attrs.setdefault("accept", "image/png,image/jpeg,image/jpg,image/webp")
...
class PublicAdmissionRegistrationForm(_StyledFormMixin, forms.ModelForm):
    ...
    parent_guardian_signature_data = forms.CharField(required=False, widget=forms.HiddenInput())
    ...
    student_signature_data = forms.CharField(required=False, widget=forms.HiddenInput())
    ...
    passport_capture_data = forms.CharField(required=False, widget=forms.HiddenInput())
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["application_fee_receipt"].required = True
        ...
        self.fields["application_fee_receipt"].label = "Admission Fee Payment Receipt"
        ...
        self.fields["application_fee_receipt"].widget.attrs.setdefault(
            "accept", ".pdf,.jpg,.jpeg,.png"
        )
    ...
    @staticmethod
    def _decode_image_data_uri(value, *, field_name):
        raw = (value or "").strip()
        if not raw:
            return None
        allowed_prefixes = {
        ...
        prefix = next((item for item in allowed_prefixes if raw.lower().startswith(item)), "")
        if not prefix:
            raise forms.ValidationError("Use the camera/signature capture provided on this page.")
        try:
            payload = base64.b64decode(raw.split(",", 1)[1], validate=True)
        except Exception as exc:
            raise forms.ValidationError("The captured image could not be read. Please capture it again.") from exc
        if len(payload) > 4 * 1024 * 1024:
            raise forms.ValidationError("Captured image is too large. Please capture it again.")
        return {
            "content": ContentFile(payload),
            "extension": allowed_prefixes[prefix],
            "data_uri": raw,
    ...
    def clean(self):
        ...
        if not cleaned.get("passport_photo") and not self._clean_text(cleaned.get("passport_capture_data")):
            self.add_error("passport_photo", "Upload or capture the applicant passport photograph before submitting.")
        if not cleaned.get("application_fee_receipt"):
            self.add_error("application_fee_receipt", "Upload the admission payment receipt before submitting.")
        try:
            passport_capture = self._decode_image_data_uri(
                cleaned.get("passport_capture_data"),
                field_name="Passport photograph",
            )
        except forms.ValidationError as exc:
            self.add_error("passport_capture_data", exc)
            passport_capture = None
        try:
            parent_signature_capture = self._decode_image_data_uri(
                cleaned.get("parent_guardian_signature_data"),
                field_name="Parent / Guardian signature",
            )
        except forms.ValidationError as exc:
            self.add_error("parent_guardian_signature_data", exc)
            parent_signature_capture = None
        try:
            student_signature_capture = self._decode_image_data_uri(
                cleaned.get("student_signature_data"),
                field_name="Student signature",
            )
        except forms.ValidationError as exc:
            self.add_error("student_signature_data", exc)
            student_signature_capture = None
        if not parent_signature_capture:
            self.add_error("parent_guardian_signature_data", "Draw and save the parent/guardian signature before submitting.")
        if not student_signature_capture:
            self.add_error("student_signature_data", "Draw and save the student signature before submitting.")
        self._passport_capture = passport_capture
        self._parent_signature_capture = parent_signature_capture
        self._student_signature_capture = student_signature_capture
        return cleaned
    ...
    def clean_application_fee_receipt(self):
        uploaded = self.cleaned_data.get("application_fee_receipt")
        if uploaded:
            validate_document_upload(uploaded)
        return uploaded
    ...
    def save(self, commit=True):
        ...
        metadata["application_payment_mode"] = "PHYSICAL"
        instance.metadata = metadata
        if not self.cleaned_data.get("passport_photo") and getattr(self, "_passport_capture", None):
            capture = self._passport_capture
            filename = f"camera-{timezone.now():%Y%m%d}-{uuid.uuid4().hex}.{capture['extension']}"
            instance.passport_photo.save(filename, capture["content"], save=False)
        if commit:
            instance.save()
            metadata = dict(instance.metadata or {})
            ...
            metadata["admission_form"] = {
                ...
                "declaration": {
                    ...
                    "parent_guardian_signature_image": (
                        self._parent_signature_capture["data_uri"] if getattr(self, "_parent_signature_capture", None) else ""
                    ),
                    ...
                    "student_signature_image": (
                        self._student_signature_capture["data_uri"] if getattr(self, "_student_signature_capture", None) else ""
                    ),
                    "student_date": (
                ...
                "documents": {
                    ...
                    "application_fee_receipt_name": getattr(
                        self.cleaned_data.get("application_fee_receipt"), "name", ""
                    ),
                    "medical_fitness_report_path": stored_medical_report,
                },
            }
            instance.metadata = metadata
            instance.save(update_fields=["applicant_name", "previous_school", "contact_name", "contact_email", "contact_phone", "payment_status", "passport_photo", "metadata", "updated_at"])
        return instance
...
class StudentClubMembershipForm(_StyledFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["student"].queryset = User.objects.filter(primary_role__code="STUDENT", is_active=True).select_related("student_profile").order_by(
            "last_name",
            "first_name",
