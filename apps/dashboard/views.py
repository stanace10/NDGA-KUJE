...
from apps.setup_wizard.services import (
    get_academic_operation_window,
    get_academic_window_state,
    get_setup_state,
    list_academic_operation_windows,
    setup_is_ready,
)
from apps.setup_wizard.feature_flags import FLAG_FIELD_MAP, get_runtime_feature_flags
from apps.setup_wizard.models import AcademicOperationWindow, RuntimeFeatureFlags
from apps.tenancy.utils import (
    cloud_student_finance_disabled,
...
from apps.dashboard.forms import (
    AcademicWindowUpdateForm,
    PublicEventImageForm,
    PublicEventPostForm,
...
def _student_finance_hidden_on_cloud():
...
def _student_dashboard_payload(request, user):
    ...
    finance_url = "" if finance_hidden else reverse("finance:student-overview")
    ...
    if not finance_hidden:
        student_quick_links.insert(
            -1,
            {
                "label": "Finance",
    ...
    if not _cloud_student_portal_limited():
        insert_at = max(len(student_quick_links) - 1, 0)
        student_quick_links[insert_at:insert_at] = [
            {
                "label": "Learning Hub",
    ...
    if not finance_hidden:
        student_dashboard_actions.insert(
            1,
            {
                "label": "Pay Fees",
                ...
                "items": [
                    {
                        ...
                        "metric": cbt_draft_count or QuestionBank.objects.filter(
                            owner=user,
                            session=current_session,
                            term=current_term,
                        ).count(),
...
def _leadership_school_payload(request):
    ...
    student_qs = student_qs.order_by(
        "last_name",
        "first_name",
    ...
    staff_qs = staff_qs.order_by("last_name", "first_name", "staff_profile__staff_id", "username")
...
class StudentPortalView(PortalPageView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["dashboard_summary_cards"] = summary_cards
        ...
        if _cloud_student_portal_limited():
            context["portal_description"] = (
                ...
                "classroom/LMS, published results, and notifications. Internal school operations stay on the school LAN."
            )
        context["portal_priority_actions"] = _build_portal_priority_actions(
...
class ITPortalView(PortalPageView):
    ...
    def get_context_data(self, **kwargs):
        ...
        now = timezone.now()
        today = timezone.localdate()
        cbt_exam_scope = Exam.objects.all()
        if current_session and current_term:
            cbt_exam_scope = cbt_exam_scope.filter(
                session=current_session,
                term=current_term,
            )
        active_cbt_count = cbt_exam_scope.filter(
            status=CBTExamStatus.ACTIVE,
        ).filter(
            Q(is_time_based=False)
            | Q(schedule_start__lte=now, schedule_end__gte=now)
            | Q(open_now=True)
        ).distinct().count()
        cbt_incidents_today = ExamAttempt.objects.filter(
            exam__in=cbt_exam_scope,
            is_locked=True,
            locked_at__date=today,
        ).count()
        pending_writeback_count = ExamAttempt.objects.filter(
            exam__in=cbt_exam_scope,
        ...
        context["portal_stat_cards"] = [
            {
                ...
                "value": active_cbt_count,
                "meta": "Papers currently inside their live examination window.",
            },
            {
                "label": "CBT Incidents Today",
                "value": cbt_incidents_today,
                "meta": "Locked attempts requiring review or an IT decision.",
            },
            {
                "label": "Writeback Issues",
                "value": pending_writeback_count,
                "meta": "Submitted attempts still awaiting complete result writeback.",
            },
            {
                "label": "Admissions Pending",
                "value": admissions_snapshot["pending"],
                "meta": "Applications currently awaiting workflow review.",
            },
        ]
        context["it_alert_rows"] = [
            {
                "label": "CBT incidents",
                "value": cbt_incidents_today,
                "url": reverse("cbt:it-lockdown-dashboard"),
                "tone": "red" if cbt_incidents_today else "green",
            },
            {
                "label": "Writeback queue",
                "value": pending_writeback_count,
                "url": reverse("cbt:it-activation-list"),
                "tone": "amber" if pending_writeback_count else "green",
            },
            {
                "label": "Unread notifications",
                "value": context["it_metrics"]["notifications_unread"],
                "url": reverse("notifications:center"),
                "tone": "amber" if context["it_metrics"]["notifications_unread"] else "green",
            },
        ]
        context["recent_admin_actions"] = list(
            AuditEvent.objects.select_related("actor")
            .filter(created_at__gte=now - timedelta(days=2))
            .order_by("-created_at")[:8]
        )
        context["system_health_rows"] = [
            {"label": "LAN application", "state": "Online", "ok": True},
            {"label": "Database", "state": "Connected", "ok": True},
            {"label": "Public website", "state": "Published", "ok": True},
        ]
...
RESULT_COMPONENT_REVIEW_KEYS = {
    AcademicOperationWindow.WindowType.RESULT_CA1: ("ca1", "CA1"),
    AcademicOperationWindow.WindowType.RESULT_CA23: ("ca23", "CA2/CA3"),
    AcademicOperationWindow.WindowType.RESULT_CA4: ("ca4", "Assignment / Projects / Practical"),
    AcademicOperationWindow.WindowType.RESULT_EXAM: ("exam", "Full Result"),
}
RESULT_COMPONENT_DRAFT = "DRAFT"
RESULT_COMPONENT_SUBMITTED = "SUBMITTED_TO_DEAN"
RESULT_COMPONENT_APPROVED = "APPROVED_BY_DEAN"
RESULT_COMPONENT_REJECTED = "REJECTED_BY_DEAN"


def _result_component_review(sheet, component_key):
    policies = getattr(sheet, "cbt_component_policies", {}) or {}
    review = policies.get("review")
    if not isinstance(review, dict):
        return {"status": RESULT_COMPONENT_DRAFT, "comment": ""}
    row = review.get(component_key)
    if not isinstance(row, dict):
        return {"status": RESULT_COMPONENT_DRAFT, "comment": ""}
    status = row.get("status") or RESULT_COMPONENT_DRAFT
    if status not in {
        RESULT_COMPONENT_DRAFT,
        RESULT_COMPONENT_SUBMITTED,
        RESULT_COMPONENT_APPROVED,
        RESULT_COMPONENT_REJECTED,
    }:
        status = RESULT_COMPONENT_DRAFT
    return {**row, "status": status}


def _set_result_component_review(sheet, component_key, *, status, actor=None, comment=""):
...
def _sync_result_sheet_status_from_component_reviews(sheet):
    policies = getattr(sheet, "cbt_component_policies", {}) or {}
    review = policies.get("review") if isinstance(policies, dict) else {}
    statuses = []
    if isinstance(review, dict):
        for component_key, _label in RESULT_COMPONENT_REVIEW_KEYS.values():
            statuses.append((review.get(component_key) or {}).get("status", RESULT_COMPONENT_DRAFT))
    if statuses and all(status == RESULT_COMPONENT_APPROVED for status in statuses):
        target = ResultSheetStatus.APPROVED_BY_DEAN
    elif any(status == RESULT_COMPONENT_REJECTED for status in statuses):
        target = ResultSheetStatus.REJECTED_BY_DEAN
    elif any(status == RESULT_COMPONENT_SUBMITTED for status in statuses):
...
class ITAcademicSetupView(PortalPageView):
    template_name = "dashboard/it_academic_setup.html"
    portal_name = "IT Manager Portal"
    portal_description = "Control academic operation windows for results and CBT workflows."
    ...
    @staticmethod
    def _prefix_for(window_type: str):
        return window_type.lower()

    def _window_form(self, *, window_type: str, data=None):
        window = get_academic_operation_window(window_type)
        return AcademicWindowUpdateForm(
            data=data,
            instance=window,
            prefix=self._prefix_for(window_type),
        )

    def _window_forms(self, *, post_data=None, active_window_type=""):
        forms = {}
        for window in list_academic_operation_windows():
            data = post_data if active_window_type == window.window_type else None
            forms[window.window_type] = self._window_form(
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        forms = kwargs.get("window_forms") or self._window_forms()
        result_state = get_academic_window_state(
            window_type=AcademicOperationWindow.WindowType.RESULTS,
            user=self.request.user,
        )
        cbt_state = get_academic_window_state(
            window_type=AcademicOperationWindow.WindowType.CBT,
            user=self.request.user,
        )
        component_window_types = [
            AcademicOperationWindow.WindowType.RESULT_CA1,
            AcademicOperationWindow.WindowType.RESULT_CA23,
            AcademicOperationWindow.WindowType.RESULT_CA4,
            AcademicOperationWindow.WindowType.RESULT_EXAM,
        ]
        session, term = current_session_term()
        component_counts = {window_type: {} for window_type in component_window_types}
        if session and term:
            sheets = ResultSheet.objects.filter(session=session, term=term).only("id", "status", "cbt_component_policies")
            for sheet in sheets:
                for window_type in component_window_types:
                    component_key, _label = RESULT_COMPONENT_REVIEW_KEYS[window_type]
                    status = _result_component_review(sheet, component_key)["status"]
                    counts = component_counts[window_type]
                    counts[status] = counts.get(status, 0) + 1
        component_rows = [
            {
                "state": get_academic_window_state(window_type=window_type, user=self.request.user),
                "form": forms[window_type],
                "window_type": window_type,
                "component_key": RESULT_COMPONENT_REVIEW_KEYS[window_type][0],
                "component_label": RESULT_COMPONENT_REVIEW_KEYS[window_type][1],
                "submitted_count": component_counts[window_type].get(RESULT_COMPONENT_SUBMITTED, 0),
                "approved_count": component_counts[window_type].get(RESULT_COMPONENT_APPROVED, 0),
                "rejected_count": component_counts[window_type].get(RESULT_COMPONENT_REJECTED, 0),
                "draft_count": component_counts[window_type].get(RESULT_COMPONENT_DRAFT, 0),
            }
            for window_type in component_window_types
        ]
        context["window_forms"] = forms
        context["result_window_form"] = forms[AcademicOperationWindow.WindowType.RESULTS]
        context["cbt_window_form"] = forms[AcademicOperationWindow.WindowType.CBT]
        context["result_window_state"] = result_state
        context["cbt_window_state"] = cbt_state
        context["component_window_rows"] = component_rows
        context["academic_window_rows"] = [result_state, cbt_state]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "return_component_to_teachers":
            return self._return_component_to_teachers(request)
        window_type = (request.POST.get("window_type") or "").strip().upper()
        valid_types = {
            AcademicOperationWindow.WindowType.RESULTS,
            AcademicOperationWindow.WindowType.RESULT_CA1,
            AcademicOperationWindow.WindowType.RESULT_CA23,
            AcademicOperationWindow.WindowType.RESULT_CA4,
            AcademicOperationWindow.WindowType.RESULT_EXAM,
            AcademicOperationWindow.WindowType.CBT,
        }
        if window_type not in valid_types:
            messages.error(request, "Invalid academic window target.")
            return redirect("dashboard:it-academic-setup")

        form = self._window_form(window_type=window_type, data=request.POST)
        if not form.is_valid():
            forms = self._window_forms(post_data=request.POST, active_window_type=window_type)
            forms[window_type] = form
            return self.render_to_response(self.get_context_data(window_forms=forms))

        window = form.save(commit=False)
        window.last_updated_by = request.user
        window.save()
        state = get_academic_window_state(window_type=window_type, user=request.user)
        log_event(
            category=AuditCategory.SYSTEM,
            event_type="ACADEMIC_WINDOW_UPDATED",
            status=AuditStatus.SUCCESS,
            actor=request.user,
            request=request,
            metadata={
                ...
                "start_at": window.start_at.isoformat() if window.start_at else "",
                "end_at": window.end_at.isoformat() if window.end_at else "",
                "status": state["status"],
            },
        )
        messages.success(request, f"{state['label']} updated. {state['summary']}")
        return redirect("dashboard:it-academic-setup")

    def _return_component_to_teachers(self, request):
        window_type = (request.POST.get("window_type") or "").strip().upper()
        if window_type not in RESULT_COMPONENT_REVIEW_KEYS:
            messages.error(request, "Choose a valid CA or exam component to return.")
            return redirect("dashboard:it-academic-setup")
        session, term = current_session_term()
        ...
        component_key, component_label = RESULT_COMPONENT_REVIEW_KEYS[window_type]
        changed = 0
        for sheet in ResultSheet.objects.filter(session=session, term=term).select_related("academic_class", "subject"):
            status = _result_component_review(sheet, component_key)["status"]
            if status not in {RESULT_COMPONENT_SUBMITTED, RESULT_COMPONENT_APPROVED}:
                continue
            _set_result_component_review(
                sheet,
                component_key,
                status=RESULT_COMPONENT_REJECTED,
                actor=request.user,
                comment=f"Returned by IT Manager so teachers can complete {component_label} score entry.",
            )
            _sync_result_sheet_status_from_component_reviews(sheet)
            changed += 1

        log_event(
            category=AuditCategory.SYSTEM,
            event_type="RESULT_COMPONENT_RETURNED_TO_TEACHERS",
            status=AuditStatus.SUCCESS,
            actor=request.user,
            request=request,
            metadata={
                ...
                "returned_count": changed,
                "session_id": str(session.id),
                "term_id": str(term.id),
            },
        )
        messages.success(request, f"{changed} {component_label} sheet(s) returned to teachers for correction.")
        return redirect("dashboard:it-academic-setup")
...
class ITPublicWebsiteSettingsView(PortalPageView):
    ...
    SECTION_META = {
        ...
        "news": {"label": "News", "description": "Publish announcements, reports, and school updates."},
        "events": {"label": "Events", "description": "Manage upcoming and past school events."},
        "event-photos": {"label": "Event Photos", "description": "Add photo galleries to specific events."},
    }
    ...
    def get_context_data(self, **kwargs):
        ...
        context["event_image_form"] = kwargs.get("event_image_form") or PublicEventImageForm()
        ...
        context["event_images"] = PublicEventImage.objects.select_related("event").order_by(
            "event__sort_order", "-event__event_date", "sort_order", "id"
        )[:160]
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "create_event_image":
            form = PublicEventImageForm(request.POST, request.FILES)
            if not form.is_valid():
                return self.render_to_response(self.get_context_data(event_image_form=form, active_section="event-photos"))
            row = form.save(commit=False)
            row.updated_by = request.user
            row.save()
            messages.success(request, "Event photo saved.")
            return self._redirect_to_section("event-photos")

        if action == "update_event_image":
            row = get_object_or_404(PublicEventImage, pk=request.POST.get("row_id"))
            form = PublicEventImageForm(request.POST, request.FILES, instance=row)
            if not form.is_valid():
                return self.render_to_response(self.get_context_data(event_image_form=form, active_section="event-photos"))
            row = form.save(commit=False)
            row.updated_by = request.user
            row.save()
            messages.success(request, "Event photo updated.")
            return self._redirect_to_section("event-photos")

        if action == "delete_event_image":
            row = get_object_or_404(PublicEventImage, pk=request.POST.get("row_id"))
            row.delete()
            messages.success(request, "Event photo deleted.")
            return self._redirect_to_section("event-photos")
