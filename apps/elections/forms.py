...
class BulkCandidateImportForm(_StyledFormMixin, forms.Form):
    entries = forms.CharField(
        required=True,
        help_text=(
            "One candidate per line. Use Position, User Lookup, optional Display Name, optional Manifesto. "
            "User Lookup accepts username, student number, or staff ID."
        ),
        widget=forms.Textarea(
            attrs={
                "rows": 8,
                "placeholder": (
                    "Head Girl, NDGAK/20/103, Candidate A, Service and discipline\n"
                    "Sports Prefect | TMP-STF-900 | Coach Ada | Stronger inter-house sports"
                ),
            }
        ),
    )
    ...
    def clean_entries(self):
        return (self.cleaned_data.get("entries") or "").strip()


class PrefectRosterImportForm(_StyledFormMixin, forms.Form):
    source_path = forms.CharField(
        required=True,
        initial="ndga/Screened Candidates For Prefects.md",
        help_text="Relative path inside this workspace to the prefect markdown file.",
    )
    is_active = forms.BooleanField(required=False, initial=True)
    update_existing = forms.BooleanField(required=False, initial=True)
    update_election_meta = forms.BooleanField(required=False, initial=True)
    prepare_voter_groups = forms.BooleanField(required=False, initial=True)
    ...
    def clean_source_path(self):
        return (self.cleaned_data.get("source_path") or "").strip()
