...
from apps.accounts.permissions import has_portal_access
...
def _form_teacher_role_account_preset(form_class):
    ...
    return {
        ...
        "password": f"ndgak/{class_code.lower()}",
    }
...
class NDGALoginForm(forms.Form):
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        if request is not None:
            self.audience = self._resolve_audience(request)
    ...
    @staticmethod
    def _resolve_audience(request):
        raw = (
            request.POST.get("audience")
            or request.GET.get("audience")
            or getattr(request, "portal_key", "")
        )
        audience = (raw or "").strip().lower()
        if audience:
            return audience
        next_url = (request.POST.get("next") or request.GET.get("next") or "").strip().lower()
        referer = (request.META.get("HTTP_REFERER") or "").strip().lower()
        host = (request.get_host() or "").strip().lower()
        if "/portal/cbt" in next_url or "/portal/cbt" in referer or "cbt." in host:
            return "cbt"
        return audience

    @staticmethod
    def request_points_to_cbt(request):
        if request is None:
            return False
        raw_values = (
            request.POST.get("audience"),
            request.GET.get("audience"),
            getattr(request, "portal_key", ""),
            request.POST.get("next"),
            request.GET.get("next"),
            request.META.get("HTTP_REFERER"),
            request.get_host(),
        )
        haystack = " ".join(str(value or "").strip().lower() for value in raw_values)
        return "audience=cbt" in haystack or "/portal/cbt" in haystack or "/cbt/" in haystack or "cbt." in haystack
    ...
    def clean(self):
        ...
        if self.request_points_to_cbt(self.request):
            self.audience = "cbt"

        if self.audience in {"election", "cbt"} and not password:
            if not user_from_identifier:
                log_login_failed(request=self.request, username=identifier)
                raise forms.ValidationError("Invalid admission number or staff ID.")
            if not user_from_identifier.is_active:
                raise forms.ValidationError("This account is inactive.")
            portal_key = "election" if self.audience == "election" else "cbt"
            if not has_portal_access(user_from_identifier, portal_key):
                log_login_failed(request=self.request, username=identifier)
                raise forms.ValidationError("Portal access is not available for this ID.")
            user_from_identifier.backend = "django.contrib.auth.backends.ModelBackend"
            self.user_cache = user_from_identifier
            return cleaned_data
        ...
        password_candidates = []
        for candidate in (password, str(password).lower(), str(password).upper()):
...
class ITStaffRegistrationForm(forms.Form):
    ...
    def clean(self):
        ...
        # Allow IT to create a subject-teacher account before every subject-class
        # load exists. This is important for newly introduced subjects such as
        # Chinese/German/Sign where the account may need to exist before the
        # timetable and ClassSubject rows are finalized.
...
class ITStudentUpdateForm(forms.Form):
    ...
    state_of_origin = forms.CharField(max_length=80, required=False)
    nationality = forms.CharField(max_length=80, required=False, initial="Nigerian")
    house = forms.CharField(max_length=80, required=False, label="House")
    community = forms.CharField(max_length=120, required=False, label="Community")
    society = forms.CharField(max_length=120, required=False, label="Society")
    ...
    def __init__(self, actor, user_instance, *args, **kwargs):
        ...
        initial.setdefault("house", self.profile_instance.house)
        initial.setdefault("community", self.profile_instance.community)
        initial.setdefault("society", self.profile_instance.society)
    ...
    def save(self):
        ...
        self.profile_instance.house = self.cleaned_data.get("house", "").strip()
        self.profile_instance.community = self.cleaned_data.get("community", "").strip()
        self.profile_instance.society = self.cleaned_data.get("society", "").strip()
