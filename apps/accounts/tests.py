...
class StageOneAccessTests(TestCase):
    ...
    def test_cbt_login_audience_allows_only_students_and_it_manager(self):
        allowed_cases = [
            ("student1", self.student),
            ("it1", self.it_manager),
        ]
        for username, user in allowed_cases:
            with self.subTest(username=username):
                client = Client()
                if user == self.student:
                    microphone_response = client.post(
                        reverse("accounts:cbt-microphone-permission"),
                        data=json.dumps({"granted": True, "secure_context": True}),
                        content_type="application/json",
                        secure=True,
                    )
                    self.assertEqual(microphone_response.status_code, 200)
                response = client.post(
                    f"{reverse('accounts:login')}?audience=cbt",
                    {
                        "username": username,
                        "password": "Password123!",
                        "audience": "cbt",
                ...
                self.assertContains(response, "/cbt/")
                self.assertTrue(
                    AuditEvent.objects.filter(
                        event_type="LOGIN_SUCCESS",
                        actor=user,
        ...
        self.assertContains(denied_response, "Microphone permission is required")
        self.assertNotIn("_auth_user_id", denied_client.session)

        blocked_cases = [
            ("teacher1", self.subject_teacher),
            ("principal1", self.principal),
            ("vp1", self.vp),
            ("bursar1", self.bursar),
        ]
        for username, user in blocked_cases:
            with self.subTest(username=username):
                client = Client()
                response = client.post(
                    f"{reverse('accounts:login')}?audience=cbt",
                ...
                self.assertEqual(response.status_code, 302)
                self.assertIn("audience=staff", response.url)
                self.assertFalse(
                    AuditEvent.objects.filter(
                        event_type="LOGIN_SUCCESS",
                        actor=user,
    ...
    def test_it_can_change_password_for_selected_student_from_management_page(self):
        ...
        response = client.post(
            reverse("accounts:it-student-password-reset", args=[self.student.id]),
            {
                "new_password": "2014",
                "confirm_password": "2014",
            },
        )

        self.assertEqual(response.status_code, 302)
        self.student.refresh_from_db()
        self.assertTrue(self.student.check_password("2014"))
    ...
    def test_teacher_cannot_change_student_password_from_management_page(self):
        StudentProfile.objects.create(
            user=self.student,
            student_number="NDGAK/26/015",
            guardian_email="parent@example.com",
        )
        client = Client(HTTP_HOST="staff.ndgakuje.org")
        client.force_login(self.subject_teacher)
        ...
        self.assertEqual(response.status_code, 302)
        self.student.refresh_from_db()
        self.assertFalse(self.student.check_password("changed"))
...
class StageNineteenSecurityTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        StudentProfile.objects.create(
            ...
            student_number="NDGAK/32/001",
        )
        StaffProfile.objects.create(
            user=cls.it_user,
            staff_id="NDGAK/STAFF/001",
        )
    ...
    def test_election_audience_accepts_id_only_login(self):
        cache.clear()
        client = Client(HTTP_HOST="election.ndgakuje.org")
        response = client.post(
            "/auth/login/?audience=election",
            {"username": "NDGAK/32/001"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "http://election.ndgakuje.org/elections/")
        self.assertEqual(int(client.session["_auth_user_id"]), self.student_user.id)

    def test_authenticated_election_login_page_keeps_it_user_in_election_shell(self):
        client = Client(HTTP_HOST="election.ndgakuje.org")
        client.force_login(self.it_user)
        response = client.get("/auth/login/?audience=election")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "http://election.ndgakuje.org/elections/")
