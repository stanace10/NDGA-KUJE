...
def academic_context_is_current(session_id, term_id):
    session, term = current_session_term()
    if session is None or term is None:
        return True
    return session_id == session.id and term_id == term.id


def active_term_edit_message():
    _session, term = current_session_term()
    term_label = term.get_name_display() if term else "the active term"
    return f"Only {term_label} records can be edited now. Use filters to view previous-term records."
...
def sheet_is_editable_by_subject_owner(user, sheet: ResultSheet):
    if not academic_context_is_current(sheet.session_id, sheet.term_id):
        return False
