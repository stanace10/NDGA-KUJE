...
class StudentSubjectScore(TimeStampedModel):
    SCORE_COMPONENT_FIELDS = ("ca1", "ca2", "ca3", "ca4", "class_participation", "objective", "theory")
    ...
    class_participation = models.DecimalField(max_digits=5, decimal_places=2, default=0, editable=False)
    ...
    def save(self, *args, **kwargs):
        if self.result_sheet_id and self.student_id:
            sheet = self.result_sheet
            calendar = SchoolCalendar.objects.filter(session=sheet.session, term=sheet.term).first()
            if calendar is not None:
                attendance_class = sheet.academic_class.instructional_class
                has_attendance = AttendanceRecord.objects.filter(
                    calendar=calendar,
                    academic_class=attendance_class,
                ...
                if has_attendance:
                    ...
                    self.class_participation = min(
                        Decimal(str(percentage)) / Decimal("10"),
                        Decimal("10.00"),
                    ).quantize(Decimal("0.01"))
                else:
                    self.class_participation = Decimal("10.00")
        self.cbt_locked_fields = self.normalized_locked_fields()
        self.cbt_component_breakdown = self.normalized_breakdown()
        if self.result_sheet_id and subject_is_excluded_from_results(self.result_sheet.subject):
            self.class_participation = Decimal("0.00")
...
class ClassCompilationStatus(models.TextChoices):
    DRAFT = "DRAFT", "Draft"
    SUBMITTED_TO_DEAN_FINAL = "SUBMITTED_TO_DEAN_FINAL", "Submitted To Dean Term Review"
    APPROVED_BY_DEAN_FINAL = "APPROVED_BY_DEAN_FINAL", "Approved By Dean Term Review"
    REJECTED_BY_DEAN_FINAL = "REJECTED_BY_DEAN_FINAL", "Rejected By Dean Term Review"
    SUBMITTED_TO_VP = "SUBMITTED_TO_VP", "Submitted To VP"
    APPROVED_BY_VP = "APPROVED_BY_VP", "Approved By VP - Ready For IT"
    REJECTED_BY_VP = "REJECTED_BY_VP", "Rejected By VP"
    PUBLISHED = "PUBLISHED", "Published"
...
class ClassResultCompilation(TimeStampedModel):
    ...
    approved_by_vp_at = models.DateTimeField(null=True, blank=True)
...
class ClassResultStudentRecord(TimeStampedModel):
    ...
    form_teacher_completed_at = models.DateTimeField(null=True, blank=True)
    hostel_supervisor_comment = models.TextField(blank=True)
    ...
    term_weight_kg = models.DecimalField(max_digits=5, decimal_places=1, null=True, blank=True)
