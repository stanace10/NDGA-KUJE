...
def mark_compilation_submitted_to_dean_final(compilation: ClassResultCompilation, actor, *, form_teacher=None):
    compilation.status = ClassCompilationStatus.SUBMITTED_TO_DEAN_FINAL
    if form_teacher is not None:
        compilation.form_teacher = form_teacher
    elif compilation.form_teacher is None:
        compilation.form_teacher = actor
...
def mark_compilation_approved_by_dean_final(compilation: ClassResultCompilation, actor, *, comment=""):
    compilation.status = ClassCompilationStatus.SUBMITTED_TO_VP
...
def mark_compilation_rejected_by_dean_final(compilation: ClassResultCompilation, actor, comment):
    compilation.status = ClassCompilationStatus.REJECTED_BY_DEAN_FINAL
    compilation.vp_actor = actor
    compilation.decision_comment = (comment or "").strip()
    compilation.save(
        update_fields=[
...
def mark_compilation_approved_by_vp(compilation: ClassResultCompilation, actor, *, comment=""):
    compilation.status = ClassCompilationStatus.APPROVED_BY_VP
    compilation.approved_by_vp_at = timezone.now()
    compilation.vp_actor = actor
    compilation.decision_comment = (comment or "").strip()
    compilation.save(
        update_fields=[
