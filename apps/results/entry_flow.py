...
def _existing_split_theory(score, field_name):
    ...
    theory_key = f"{field_name}_theory"
    if theory_key in breakdown:
        return score.breakdown_value(theory_key)
...
def _existing_locked_or_value(score, breakdown_key, field_name):
    if score is None:
        return ZERO
    if breakdown_key in score.normalized_breakdown():
        return score.breakdown_value(breakdown_key)
...
def _component_has_cbt_score(score, breakdown_key, field_name):
    if score is None:
        return False
    return breakdown_key in score.normalized_breakdown() or score.is_component_locked(field_name)
...
def row_component_state(score, policies):
    ...
    ca1_objective = (
        _existing_locked_or_value(score, "ca1_objective", "ca1")
        if ca1_objective_locked or not policies["ca1"]["enabled"]
        else ZERO
    )
    ...
    ca2_objective = (
        _existing_locked_or_value(score, "ca2_objective", "ca2")
        if ca23_objective_locked or not policies["ca23"]["enabled"]
        else ZERO
    )
    objective_auto = (
        _existing_locked_or_value(score, "objective_auto", "objective")
        if exam_objective_locked or not policies["exam"]["enabled"]
        else ZERO
    )
    exam_theory = decimal_value(score.theory if score else ZERO)
    objective_display = objective_auto.quantize(DECIMAL_2)
    ...
    return {
        ...
        "exam": {
            ...
            "objective": decimal_text(objective_display),
...
def read_sheet_policies_from_post(sheet, post, existing_scores):
    raw_policies = getattr(sheet, "cbt_component_policies", {}) or {}
    ...
    if isinstance(raw_policies, dict):
        for extra_key in ("review", "emergency_entry_windows"):
            if extra_key in raw_policies:
                policies[extra_key] = raw_policies[extra_key]
...
def build_posted_score_bundle(*, current_score, post, student_id, policies, actor, allowed_components=None):
    allowed_components = set(allowed_components or POLICY_SCORE_FIELDS)
    ...
    if "ca1" not in allowed_components:
        posted_scores["ca1"] = decimal_value(getattr(current_score, "ca1", ZERO) if current_score else ZERO)
    elif policies["ca1"]["enabled"]:
        has_cbt_objective = _component_has_cbt_score(current_score, "ca1_objective", "ca1")
        if has_cbt_objective:
            ...
            manual_total = posted_scores["ca1"]
    ...
    if "ca23" not in allowed_components:
        posted_scores["ca2"] = decimal_value(getattr(current_score, "ca2", ZERO) if current_score else ZERO)
        posted_scores["ca3"] = decimal_value(getattr(current_score, "ca3", ZERO) if current_score else ZERO)
    elif policies["ca23"]["enabled"]:
        has_cbt_objective = _component_has_cbt_score(current_score, "ca2_objective", "ca2")
        if has_cbt_objective:
        ...
        objective_score = (
            objective_score
            if has_cbt_objective
            else decimal_value(post.get(f"ca2_{student_id}"), posted_scores["ca2"])
        )
        ...
        if has_cbt_objective:
            ...
            breakdown_updates["ca2_objective"] = ZERO
    ...
    if "ca4" not in allowed_components:
        posted_scores["ca4"] = decimal_value(getattr(current_score, "ca4", ZERO) if current_score else ZERO)
    elif policies["ca4"]["enabled"]:
    ...
    if "exam" not in allowed_components:
        posted_scores["objective"] = decimal_value(getattr(current_score, "objective", ZERO) if current_score else ZERO)
        posted_scores["theory"] = decimal_value(getattr(current_score, "theory", ZERO) if current_score else ZERO)
    elif policies["exam"]["enabled"]:
        has_cbt_objective = _component_has_cbt_score(current_score, "objective_auto", "objective")
        if has_cbt_objective:
            ...
            objective_score = decimal_value(
                ...
                current_score.objective if current_score else ZERO,
            )
        theory_score = decimal_value(
        ...
        breakdown_updates["objective_display_raw"] = objective_score
    else:
        if "objective" in locked_fields and current_score is not None:
            posted_scores["objective"] = decimal_value(current_score.objective)
        else:
            objective_score = posted_scores["objective"]
            objective_max = decimal_value(policies["exam"]["objective_max"])
            if objective_score < ZERO or objective_score > objective_max:
                raise ValidationError(
                    {"objective": f"Exam objective must be between 0 and {objective_max}."}
                )
            breakdown_updates["objective_display_raw"] = objective_score
    ...
    payload = compute_grade_payload(
        ...
        class_participation=(current_score.class_participation if current_score else ZERO),
