...
class PDFStageEightTests(TestCase):
    ...
    def test_exam_only_languages_are_excluded_from_term_report(self):
        from apps.results.analytics import _student_average_rows

        chinese = Subject.objects.create(name="Chinese", code="CHN")
        ClassSubject.objects.create(academic_class=self.academic_class, subject=chinese)
        sheet = ResultSheet.objects.create(
        ...
        response = self._client("student.ndgakuje.org", self.student).get(
            f"/pdfs/student/reports/{self.published_compilation.id}/"
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "English Language")
        self.assertNotContains(response, "Chinese")
        analytics_rows = _student_average_rows(
            session=self.session,
            term=self.first_term,
            academic_class=self.academic_class,
        )
        self.assertEqual(len(analytics_rows), 1)
        self.assertEqual(analytics_rows[0]["actual_subject_count"], 1)
