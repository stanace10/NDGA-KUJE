...
from apps.pdfs.services import (
    ...
    generate_cumulative_report_pdf,
...
from apps.finance.models import StudentCharge
...
def _student_result_fee_lock(student, *, compilation):
    ...
    charges_exist = StudentCharge.objects.filter(
        ...
        term__in=[compilation.term, None]
    ).exists()
    if not charges_exist:
        return {"locked": False, "outstanding": Decimal("0.00"), "message": ""}
    overview = student_finance_overview(student=student, session=compilation.session, term=compilation.term)
    outstanding = overview.get("total_outstanding") or Decimal("0.00")
    locked = outstanding > Decimal("0.00")
    return {
        "locked": locked,
        "outstanding": outstanding,
        "message": (
            "Result access is blocked until outstanding school fees are cleared."
            if locked
            else ""
        ),
    }
...
def _ensure_student_report_access(request, *, compilation):
    fee_lock = _student_result_fee_lock(request.user, compilation=compilation)
    if fee_lock["locked"]:
        messages.error(
            request,
            f"{fee_lock['message']} Outstanding balance: {fee_lock['outstanding']}.",
        )
        return False
    if not SchoolProfile.load().require_result_access_pin:
        return True
...
class StudentReportsView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for compilation in compilations:
            ...
            fee_lock = _student_result_fee_lock(self.request.user, compilation=compilation)
            compilation_rows.append(
                {
                    ...
                    "fee_message": fee_lock["message"],
                }
            )
            if featured_compilation is None:
                featured_compilation = compilation
                featured_locked = (pin_required and not pin_verified) or fee_lock["locked"]
...
class StudentCumulativeReportDownloadView(LoginRequiredMixin, UserPassesTestMixin, View):
    def test_func(self):
        return self.request.user.has_role(ROLE_STUDENT)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if compilation.term.name != "THIRD":
            messages.error(request, "Cumulative promotion report is issued from the Third Term result.")
            return redirect("pdfs:student-reports")
        if not _ensure_student_report_access(request, compilation=compilation):
            return redirect("pdfs:student-reports")
        try:
            pdf_bytes, _artifact = generate_cumulative_report_pdf(
                request=request,
                student=request.user,
                compilation=compilation,
                generated_by=request.user,
        ...
        log_pdf_generation(
            ...
            request=request,
            metadata={
                "document_type": "CUMULATIVE_REPORT",
                "student_id": str(request.user.id),
                "compilation_id": str(compilation.id),
            },
        )
        filename = f"NDGA-Cumulative-Report-{request.user.username}-{compilation.session.name}.pdf"
        return _pdf_download_response(pdf_bytes=pdf_bytes, filename=filename)
...
class StaffCumulativeReportDownloadView(LoginRequiredMixin, UserPassesTestMixin, View):
    def test_func(self):
        return has_any_role(self.request.user, STAFF_REPORT_DOWNLOAD_ROLES)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if compilation.term.name != "THIRD":
            ...
            return redirect(resolve_role_home_url(request.user, request=request))
        if not can_staff_download_term_report(user=request.user, compilation=compilation):
            messages.error(request, "You are not authorized to export this cumulative report.")
            return redirect(resolve_role_home_url(request.user, request=request))
        ...
        filename = f"NDGA-Cumulative-Report-{student.username}-{compilation.session.name}.pdf"
        return _pdf_download_response(pdf_bytes=pdf_bytes, filename=filename)
