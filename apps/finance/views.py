...
from apps.finance.prospectus import (
    core_student_fee_rows,
    ensure_prospectus_charges,
    prospectus_for_class,
)
...
from apps.notifications.models import EmailReplyThread, EmailThreadScope, Notification, NotificationCategory
...
CORE_STUDENT_FEE_ROWS = core_student_fee_rows()
...
def _ensure_default_core_charges(*, session, term, actor=None):
    ensure_prospectus_charges(session=session, term=term, actor=actor)
...
class StudentFinanceOverviewView(StudentFinanceAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        prospectus_fee_schedule = prospectus_for_class(current_class.code if current_class else "", new_intake=False)
        student_photo_url = ""
        ...
        context.update(
            {
                ...
                "pixpay_school_fees_url": getattr(settings, "PIXPAY_SCHOOL_FEES_URL", ""),
    ...
    def post(self, request, *args, **kwargs):
        messages.error(
            request,
            "Online payment inside the school portal is no longer active. Please use PixPay and check this page after bursary records the payment.",
        )
...
class BursarChargeManagementView(BursarAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["prospectus_schedules"] = [
            {
                "class_label": row.display_name or row.code,
                "schedule": prospectus_for_class(row.code, new_intake=False),
            }
            for row in context["class_options"]
        ]
...
class BursarPaymentManagementView(BursarAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["prospectus_schedules"] = [
            ...
            for row in _instructional_class_options()
        ]
        context["current_session"] = session
        context["current_term"] = term
...
class BursarStudentFinanceDetailView(BursarAccessMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "init_gateway_link":
            messages.error(request, "Gateway payment links are no longer active. Record confirmed PixPay payments instead.")
            return redirect("finance:bursar-student-finance", student_id=self.student.id)

        if action == "verify_gateway_reference":
            messages.error(request, "Gateway verification is no longer active. Record confirmed PixPay payments instead.")
            return redirect("finance:bursar-student-finance", student_id=self.student.id)
...
class BursarMessagingView(BursarAccessMixin, TemplateView):
    ...
    def _reply_threads():
        return (
            EmailReplyThread.objects.filter(scope=EmailThreadScope.FINANCE)
            .select_related("created_by")
            .order_by("-last_message_at", "-created_at")[:24]
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["reply_threads"] = self._reply_threads()
        context["reply_feature_enabled"] = bool(
    ...
    def post(self, request, *args, **kwargs):
        ...
        send_email_event(
            ...
            request=request,
            enable_replies=True,
            reply_thread_scope=EmailThreadScope.FINANCE,
            metadata={
                "event": "BURSAR_MESSAGE",
                "target_scope": form.cleaned_data["target_scope"],
