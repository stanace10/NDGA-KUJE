...
from apps.academics.grade_scale import NDGA_GRADE_SCALE
...
class GradeScaleDefaultsForm(forms.Form):
    DEFAULT_SCALE = tuple(
        (row["grade"], row["min_score"], row["max_score"], row["sort_order"])
    ...
    @staticmethod
    def _field_prefix(grade):
        return str(grade).lower()

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        for grade in self.GRADE_ORDER:
            prefix = self._field_prefix(grade)
            self.fields[f"{prefix}_min"] = forms.IntegerField(
                min_value=0,
                max_value=100,
                label=f"{grade} Minimum",
                required=False,
            )
            self.fields[f"{prefix}_max"] = forms.IntegerField(
                min_value=0,
                max_value=100,
                label=f"{grade} Maximum",
                required=False,
            )
        ...
        for grade in self.GRADE_ORDER:
            ...
            self.fields[f"{prefix}_min"].initial = values["min"]
            self.fields[f"{prefix}_max"].initial = values["max"]
        self.fields["apply_defaults"].widget.attrs.update(
            {"class": "h-4 w-4 rounded border-slate-300 text-ndga-navy"}
        )
        for grade in self.GRADE_ORDER:
            prefix = self._field_prefix(grade)
            self.fields[f"{prefix}_min"].widget.attrs.update(
                {"class": "w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"}
            )
            self.fields[f"{prefix}_max"].widget.attrs.update(
                {"class": "w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"}
            )
    ...
    def clean(self):
        ...
        if not self.errors and all(grade in grade_ranges for grade in self.GRADE_ORDER):
            first_grade = self.GRADE_ORDER[0]
            last_grade = self.GRADE_ORDER[-1]
            if grade_ranges[first_grade]["max_score"] != 100:
                self.add_error(f"{self._field_prefix(first_grade)}_max", f"{first_grade} maximum must be 100.")
            if grade_ranges[last_grade]["min_score"] != 0:
                self.add_error(f"{self._field_prefix(last_grade)}_min", f"{last_grade} minimum must be 0.")
