...
from apps.setup_wizard.models import (
    AcademicOperationWindow,
    SetupStateCode,
    SystemSetupState,
)
...
def get_setup_state():
    return (
        SystemSetupState.objects.select_related("current_session", "current_term")
        .filter(singleton_id=1)
        .first()
        or SystemSetupState.get_solo()
    )


def setup_is_ready():
    cache_key = "setup_wizard:setup_is_ready:v1"
    cached = cache.get(cache_key)
    if cached is not None:
        return bool(cached)
    try:
        ready = get_setup_state().is_ready
    except (OperationalError, ProgrammingError):
        return False
    cache.set(cache_key, bool(ready), 5)
    return bool(ready)
...
def get_academic_operation_window(window_type: str):
    window, _created = AcademicOperationWindow.objects.get_or_create(
...
def list_academic_operation_windows():
    window_types = (
    ...
    return [get_academic_operation_window(window_type) for window_type in window_types]


def user_must_obey_academic_window(user):
    if not getattr(user, "is_authenticated", False):
        return False
    return not (
        getattr(user, "is_superuser", False)
        or user.has_role(ROLE_IT_MANAGER)
        or user.has_role(ROLE_PRINCIPAL)
    )


def get_academic_window_state(*, window_type: str, user=None, at_time=None):
    ...
    start_at = timezone.localtime(window.start_at) if window.start_at else None
    end_at = timezone.localtime(window.end_at) if window.end_at else None

    status = "DISABLED"
    is_open = False
    if window.is_enabled and start_at and end_at:
        if start_at <= local_now <= end_at:
            status = "OPEN"
            is_open = True
        elif local_now < start_at:
            status = "SCHEDULED"
        else:
            status = "EXPIRED"

    label = window.get_window_type_display()
    if status == "OPEN":
        summary = f"{label} is open until {end_at.strftime('%d %b %Y %H:%M')}."
    elif status == "SCHEDULED":
        summary = f"{label} is scheduled from {start_at.strftime('%d %b %Y %H:%M')}."
    elif status == "EXPIRED":
        summary = f"{label} closed at {end_at.strftime('%d %b %Y %H:%M')}."
    else:
        summary = f"{label} is currently closed."

    requires_window = user_must_obey_academic_window(user) if user is not None else False
    can_manage = bool(
        user is not None
        and getattr(user, "is_authenticated", False)
        and (getattr(user, "is_superuser", False) or user.has_role(ROLE_IT_MANAGER))
    )
    ...
    return {
        ...
        "is_bypassed_for_user": bool(user is not None and not requires_window),
    }


def is_academic_window_open(*, window_type: str, user=None, at_time=None):
    if user is not None and not user_must_obey_academic_window(user):
        return True
    state = get_academic_window_state(window_type=window_type, user=user, at_time=at_time)
    return bool(state["is_open"])


def require_academic_window(*, window_type: str, user, action_label: str):
    state = get_academic_window_state(window_type=window_type, user=user)
    if not state["requires_window"] or state["is_open"]:
        return state
    raise ValidationError(
        f"{state['label']} is closed. IT Manager must open it in Academic Setup before {action_label}."
    )
