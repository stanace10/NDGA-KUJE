...
class SetupWizardView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        return self.request.user.has_role(ROLE_IT_MANAGER) or self.request.user.is_superuser
...
class SessionTermManagementView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        setup_state = get_setup_state()
        can_progress = self.request.user.has_role(ROLE_IT_MANAGER) or self.request.user.is_superuser
    ...
    def post(self, request, *args, **kwargs):
        ...
        is_it_manager = request.user.has_role(ROLE_IT_MANAGER) or request.user.is_superuser
