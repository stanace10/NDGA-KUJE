...
from apps.notifications.forms import EmailReplyForm, MediaBroadcastForm
...
from apps.notifications.services import (
    ...
    extract_whatsapp_phones,
    fetch_brevo_inbound_attachment,
    ingest_brevo_inbound_payload,
    send_email_event,
    send_whatsapp_event,
)
...
def _can_manage_general_replies(user):
    return has_any_role(user, {ROLE_IT_MANAGER, ROLE_VP, ROLE_PRINCIPAL})


def _can_manage_finance_replies(user):
    return has_any_role(user, {ROLE_BURSAR, ROLE_VP, ROLE_PRINCIPAL})


def _reply_thread_queryset(user):
    allowed_scopes = []
    if _can_manage_general_replies(user):
        allowed_scopes.append(EmailThreadScope.GENERAL)
    if _can_manage_finance_replies(user):
        allowed_scopes.append(EmailThreadScope.FINANCE)
    if not allowed_scopes:
        return EmailReplyThread.objects.none()
    return (
        EmailReplyThread.objects.filter(scope__in=allowed_scopes)
        .select_related("created_by")
        .prefetch_related("messages", "messages__created_by")
...
class MediaCenterView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["reply_threads"] = _reply_thread_queryset(self.request.user).filter(
            scope=EmailThreadScope.GENERAL,
        )[:24]
        context["reply_feature_enabled"] = _reply_feature_enabled()
        context["reply_domain"] = str(getattr(settings, "NOTIFICATIONS_REPLY_DOMAIN", "") or "").strip()
...
class EmailReplyThreadDetailView(LoginRequiredMixin, TemplateView):
    template_name = "notifications/email_thread_detail.html"

    def dispatch(self, request, *args, **kwargs):
        self.thread = get_object_or_404(
            _reply_thread_queryset(request.user),
            pk=kwargs["thread_id"],
        )
        return super().dispatch(request, *args, **kwargs)

    def _form(self, data=None):
        return EmailReplyForm(data=data)

    def _back_url(self):
        if self.thread.scope == EmailThreadScope.FINANCE:
            return reverse("finance:bursar-messaging"), "Finance Messaging"
        return reverse("notifications:media-center"), "Messaging Center"

    def _message_rows(self):
        rows = []
        for message in self.thread.messages.all():
            attachments = []
            for index, item in enumerate(message.attachments or [], start=1):
                if not isinstance(item, dict):
                    continue
                download_token = str(
                    item.get("DownloadToken")
                    or item.get("downloadToken")
                    or item.get("Token")
                    or ""
                ).strip()
                filename = (
                ...
                attachments.append(
                    {
                        ...
                        "content_type": str(item.get("ContentType") or item.get("contentType") or "").strip(),
                    }
                )
            rows.append(
                {
                    "message": message,
                    "is_inbound": message.direction == EmailReplyMessageDirection.INBOUND,
                    "body": (
                        (message.extracted_text or "").strip()
                        or (message.body_text or "").strip()
                        or "No readable body was extracted from this email."
                    ),
                    "signature": (message.extracted_signature or "").strip(),
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        back_url, back_label = self._back_url()
        context["thread"] = self.thread
        context["reply_form"] = kwargs.get("reply_form") or self._form()
        context["message_rows"] = self._message_rows()
    ...
    def post(self, request, *args, **kwargs):
        ...
        result = send_email_event(
            to_emails=[self.thread.recipient_email],
            ...
            metadata={
                "event": "EMAIL_THREAD_REPLY",
                "thread_id": str(self.thread.id),
                "thread_scope": self.thread.scope,
            },
        )
        if getattr(result, "success", False):
            messages.success(request, f"Reply sent to {self.thread.recipient_email}.")
        else:
            messages.error(
                request,
                f"Reply could not be sent. {getattr(result, 'detail', 'Please check the email configuration.')}",
            )
        return redirect("notifications:reply-thread-detail", thread_id=self.thread.id)


class EmailReplyAttachmentDownloadView(LoginRequiredMixin, View):
    def dispatch(self, request, *args, **kwargs):
        self.thread = get_object_or_404(
            _reply_thread_queryset(request.user),
            pk=kwargs["thread_id"],
    ...
    def get(self, request, *args, **kwargs):
        message = get_object_or_404(self.thread.messages, pk=kwargs["message_id"])
        download_token = (kwargs["download_token"] or "").strip()
        ...
        for item in message.attachments or []:
            ...
            if candidate != download_token:
                continue
            token_found = True
            attachment_name = (
                str(item.get("Name") or item.get("Filename") or item.get("name") or "").strip()
                or attachment_name
            )
            break

        if not token_found:
            messages.error(request, "That attachment could not be found for this reply.")
            return redirect("notifications:reply-thread-detail", thread_id=self.thread.id)

        try:
            payload = fetch_brevo_inbound_attachment(download_token)
        ...
        content_disposition = (payload.get("content_disposition") or "").strip()
        response["Content-Disposition"] = content_disposition or f'attachment; filename="{attachment_name}"'
        return response
...
class BrevoInboundWebhookView(View):
    def post(self, request, *args, **kwargs):
        expected_token = str(getattr(settings, "BREVO_INBOUND_WEBHOOK_BEARER_TOKEN", "") or "").strip()
        if not expected_token:
            return JsonResponse({"detail": "Inbound email webhook is not configured."}, status=503)
        ...
        summary = ingest_brevo_inbound_payload(payload)
        return JsonResponse({"detail": "ok", **summary})
