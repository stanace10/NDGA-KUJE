...
class ConsoleEmailProvider:
    ...
    def send(self, *, to_emails, subject, body_text, body_html="", attachments=None, reply_to=None):
        ...
        message = EmailMultiAlternatives(
            ...
            to=to_emails,
            reply_to=[reply_to.get("email")] if reply_to and reply_to.get("email") else None,
        )
        if body_html:
...
class BrevoEmailProvider:
    ...
    def send(self, *, to_emails, subject, body_text, body_html="", attachments=None, reply_to=None):
        ...
        if reply_to and reply_to.get("email"):
            payload["replyTo"] = {
                "email": reply_to.get("email"),
                "name": reply_to.get("name") or settings.BREVO_SENDER_NAME,
            }
        if attachments:
            payload["attachment"] = [
                {
        ...
        try:
            with urlopen(request, timeout=timeout_seconds) as response:
                ...
                ok = 200 <= status < 300
                response_body = response.read().decode("utf-8", errors="ignore") or "{}"
                try:
                ...
                return EmailSendResult(
                    ...
                    external_message_id=str(parsed.get("messageId") or "").strip(),
                )
