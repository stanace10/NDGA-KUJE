...
def _attendance_class_for_assignment(assignment):
    academic_class = getattr(assignment, "academic_class", None)
...
class FormTeacherAttendanceView(RoleRestrictedView):
    ...
    def _build_context(self):
        ...
        if page_obj.object_list:
            attendance_class = _attendance_class_for_assignment(assignment)
            student_ids = [row.student_id for row in page_obj.object_list]
            absent_ids = set(AttendanceRecord.objects.filter(
                calendar=calendar,
            ...
            context["present_student_ids"] = set(student_ids) - absent_ids
...
class FormTeacherWeeklyAttendanceView(RoleRestrictedView):
    ...
    def _build_context(self):
        ...
        if page_obj.object_list and week_days:
            attendance_class = _attendance_class_for_assignment(assignment)
            student_ids = [row.student_id for row in page_obj.object_list]
            selectable_days = [row for row in week_days if row["selectable"]]
            date_rows = [row["date"] for row in selectable_days]
            absent_rows = set(
            ...
            for enrollment in page_obj.object_list:
                enrollment.week_presence_tokens = {
                    row["token"]
                    for row in selectable_days
                    if (enrollment.student_id, row["date"]) not in absent_rows
                }
