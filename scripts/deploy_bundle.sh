...
BUNDLE_PATH="${1:-${NDGA_DEPLOY_BUNDLE:-}}"
...
if [ -z "$BUNDLE_PATH" ]; then
  log "Deploy bundle path was not provided."
  exit 1
fi
if [ ! -f "$BUNDLE_PATH" ]; then
  log "Deploy bundle not found: $BUNDLE_PATH"
  exit 1
fi
if [ ! -f "$ENV_FILE" ]; then
...
log "Extracting deploy bundle"
tar -xf "$BUNDLE_PATH" -C "$PROJECT_DIR"
