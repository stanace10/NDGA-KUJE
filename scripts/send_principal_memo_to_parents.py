...
from apps.notifications.models import EmailThreadScope
...
def _requested_recipient_list():
    raw = os.getenv("NDGA_PARENT_EMAILS", "")
    if not raw.strip():
        return []
    emails = []
    seen = set()
...
def _recipient_emails():
    requested = _requested_recipient_list()
    if requested:
        return requested
