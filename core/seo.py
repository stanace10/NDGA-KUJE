...
def _is_indexable(request):
    if request.method != "GET" or getattr(request.user, "is_authenticated", False) or current_portal_key(request) != "landing":
        return False
    if request.path in _indexable_paths():
        return True
    if request.path.startswith("/news/"):
        slug = request.path.strip("/").split("/")[-1]
        return bool(get_public_news_item(slug))
    if request.path.startswith("/events/"):
        slug = request.path.strip("/").split("/")[-1]
        return bool(get_public_event_item(slug))
    return (
        False
    )
...
def _public_meta(request):
    ...
    if request.path.startswith("/events/"):
        slug = request.path.strip("/").split("/")[-1]
        event_item = get_public_event_item(slug)
        if event_item:
            return {
                "site_name": f"{event_item['title']} | {PUBLIC_SITE_NAME}",
                "description": event_item["summary"],
            }
    return {
        "site_name": PUBLIC_SITE_NAME,
        "description": PUBLIC_DESCRIPTION,
...
def sitemap_xml(request):
    ...
    urls = []
    dynamic_paths = [f"/news/{item['slug']}/" for item in get_public_news()] + [
        f"/events/{item['slug']}/" for item in get_public_events()
    ]
    for path in sorted(set(_indexable_paths()) | set(dynamic_paths)):
