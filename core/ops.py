...
def _cache_ready(alias="default"):
    try:
        backend = caches[alias]
        key = f"ndga_readyz_ping:{alias}"
        backend.set(key, "ok", timeout=10)
        return backend.get(key) == "ok"
    except Exception:  # pragma: no cover - runtime dependency check
        return False
...
def collect_ops_runtime_snapshot():
    checks = {
        ...
        "cache": _cache_ready("default"),
        "redis_state": _cache_ready("state"),
    }
