...
env = environ.Env(
    ...
    CBT_MICROPHONE_REQUIRED=(bool, False),
    CBT_BROWSER_BREACH_DETECTION_ENABLED=(bool, True),
    ...
    NOTIFICATIONS_EMAIL_REPLY_ENABLED=(bool, False),
    NOTIFICATIONS_REPLY_DOMAIN=(str, ""),
    BREVO_API_KEY=(str, ""),
    BREVO_SENDER_NAME=(str, "NDGA"),
    BREVO_INBOUND_WEBHOOK_BEARER_TOKEN=(str, ""),
    ...
    AI_PROVIDER_ORDER=(str, "gemini,openai,groq,huggingface"),
    GROQ_API_KEY=(str, ""),
    GROQ_MODEL=(str, "llama-3.1-8b-instant"),
    GEMINI_API_KEY=(str, ""),
    GEMINI_USER_API_KEY=(str, ""),
...
EXAM_SAFE_MODE = env.bool("EXAM_SAFE_MODE", default=False)
DATABASES["default"]["CONN_MAX_AGE"] = (
    0
    if EXAM_SAFE_MODE
    else max(env.int("DATABASE_CONN_MAX_AGE", default=env.int("CONN_MAX_AGE", default=60)), 0)
)
DATABASES["default"]["DISABLE_SERVER_SIDE_CURSORS"] = EXAM_SAFE_MODE

REDIS_STATE_URL = env("REDIS_STATE_URL", default="redis://127.0.0.1:6379/4")
...
if "state" not in CACHES:
    CACHES["state"] = CACHES["default"].copy()

SESSION_ENGINE = "django.contrib.sessions.backends.cached_db"
SESSION_CACHE_ALIAS = "state"
...
CELERY_TASK_ROUTES = {
    "cbt.flush_attempt_live_state": {"queue": "critical"},
    "cbt.persist_lockdown_audit": {"queue": "critical"},
    "elections.*": {"queue": "default"},
    "finance.*": {"queue": "reporting"},
    "notifications.*": {"queue": "reporting"},
    "sync.*": {"queue": "default"},
    "setup.*": {"queue": "reporting"},
}
CELERY_WORKER_SEND_TASK_EVENTS = not EXAM_SAFE_MODE
CELERY_TASK_SEND_SENT_EVENT = not EXAM_SAFE_MODE
...
FEATURE_FLAGS = {
    ...
    "EXAM_SAFE_MODE": EXAM_SAFE_MODE,
}

ENABLE_BACKGROUND_REPORTS = not EXAM_SAFE_MODE
ENABLE_EMAIL_TASKS = not EXAM_SAFE_MODE
ENABLE_ANALYTICS_TASKS = not EXAM_SAFE_MODE
ENABLE_NON_EXAM_BEAT_TASKS = not EXAM_SAFE_MODE
...
PORTAL_SUBDOMAINS = {
    ...
    "hostel": env("NDGA_HOSTEL_HOST", default=f"hostel.{NDGA_BASE_DOMAIN}"),
...
NOTIFICATIONS_FROM_EMAIL = env(
    ...
    default="office@ndgakuje.org",
)
NOTIFICATIONS_EMAIL_REPLY_ENABLED = env("NOTIFICATIONS_EMAIL_REPLY_ENABLED", default=False)
NOTIFICATIONS_REPLY_DOMAIN = env(
    "NOTIFICATIONS_REPLY_DOMAIN",
    default=f"reply.{NDGA_BASE_DOMAIN}",
)
DEFAULT_FROM_EMAIL = NOTIFICATIONS_FROM_EMAIL
SERVER_EMAIL = NOTIFICATIONS_FROM_EMAIL
BREVO_API_KEY = env("BREVO_API_KEY", default="")
BREVO_SENDER_NAME = env("BREVO_SENDER_NAME", default="NDGA")
BREVO_INBOUND_WEBHOOK_BEARER_TOKEN = env("BREVO_INBOUND_WEBHOOK_BEARER_TOKEN", default="")
...
PIXPAY_SCHOOL_FEES_URL = env("PIXPAY_SCHOOL_FEES_URL", default="https://portal.pixpay.ng/ProspectiveFees")
ADMISSION_PAYMENT_BANK_NAME = env("ADMISSION_PAYMENT_BANK_NAME", default="Fidelity Bank")
ADMISSION_PAYMENT_ACCOUNT_NUMBER = env("ADMISSION_PAYMENT_ACCOUNT_NUMBER", default="5620087065")
ADMISSION_PAYMENT_ACCOUNT_NAME = env("ADMISSION_PAYMENT_ACCOUNT_NAME", default="Notre dame girls academy kuje")
ADMISSION_APPLICATION_FEE_AMOUNT = Decimal(env("ADMISSION_APPLICATION_FEE_AMOUNT", default="5500.00"))
...
CBT_MICROPHONE_REQUIRED = env.bool("CBT_MICROPHONE_REQUIRED", default=False)
CBT_BROWSER_BREACH_DETECTION_ENABLED = env.bool(
    "CBT_BROWSER_BREACH_DETECTION_ENABLED",
    default=True,
)
CBT_SEB_ENFORCEMENT_ENABLED = env.bool(
    "CBT_SEB_ENFORCEMENT_ENABLED",
    default=False,
)
...
CLOUD_STUDENT_FINANCE_DISABLED = env.bool(
    "CLOUD_STUDENT_FINANCE_DISABLED",
    default=(SYNC_NODE_ROLE == "CLOUD"),
)
STUDENT_FEE_STATUS_AVAILABLE = env.bool(
    "STUDENT_FEE_STATUS_AVAILABLE",
    default=False,
)
...
BIRTHDAY_WISHES_ENABLED = env.bool("BIRTHDAY_WISHES_ENABLED", default=False)
...
CELERY_BEAT_SCHEDULE = {
    ...
    "elections-auto-close-due": {
        "task": "elections.auto_close_due",
        "schedule": 30.0,
    },
    "notifications-send-daily-birthday-wishes": {
        "task": "notifications.send_daily_birthday_wishes",
        "schedule": crontab(hour=0, minute=0),
    },
    "notifications-send-birthday-catchup-wishes": {
        "task": "notifications.send_birthday_catchup_wishes",
