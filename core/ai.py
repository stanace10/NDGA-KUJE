...
DEFAULT_AI_PROVIDER_ORDER = ("gemini", "openai", "groq", "huggingface")
...
def _gemini_json_response_with_fallback(*, model, system_prompt, user_prompt):
    keys = [
        str(_setting("GEMINI_USER_API_KEY", "")).strip(),
        str(_setting("GEMINI_API_KEY", "")).strip(),
    ]
    attempted = set()
    for api_key in keys:
...
def ai_json_response(*, system_prompt, user_prompt):
    provider_map = {
        ...
        "gemini": lambda: _gemini_json_response_with_fallback(
            model=str(_setting("GEMINI_MODEL", "gemini-3.5-flash")).strip() or "gemini-3.5-flash",
            system_prompt=system_prompt,
            user_prompt=user_prompt,
        ),
