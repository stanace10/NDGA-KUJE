...
if [ "$DJANGO_SETTINGS_MODULE" = "core.settings.local" ]; then
  echo "Refusing to start LAN container with core.settings.local" >&2
  exit 1
fi
...
if [ -z "${DJANGO_SECRET_KEY:-}" ] || \
  ...
  echo "DJANGO_SECRET_KEY must be a real LAN deployment secret of at least 32 characters." >&2
  exit 1
fi
